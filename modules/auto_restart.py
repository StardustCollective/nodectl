...
class AutoRestart():
    ...
    def on_boot_handler(self):
        ...
        while True:
            ...
            if 60 <= uptime_seconds <= 70:
                self._print_log_msg("warning",f"auto_restart - on_boot_handler - thread [{self.thread_profile}] - restarting auto_restart service after 60 seconds to ensure stability")
                raise Exception("auto_restart forced stability restart")
            elif uptime_seconds < 60: 
                for n in range(0,10):
                    self._print_log_msg("debug",f"auto_restart - on_boot_handler - thread [{self.thread_profile}] - sleeping [{n}] of 10 seconds before next on_boot check")
                    sleep(1)  
            else: 
                self.on_boot_handler_check = False
                break

        self._print_log_msg("debug",f"auto_restart - on_boot_handler - thread [{self.thread_profile}] - uptime in seconds [{uptime_seconds}]")
        
                      
    # SETUP  
    def build_class_objs(self):
        self._print_log_msg("info",f"auto_restart - thread [{self.thread_profile}] -> build node services class obj")
        ...
        self._print_log_msg("info","auto_restart -> build version class obj")  
        ...
        self._print_log_msg("info",f"auto_restart - thread [{self.thread_profile}] - starting node services...")
        command_obj = {
            "caller": "cli",
            "auto_restart": True,
        ...
        self._print_log_msg("debug","auto_restart -> start_node_service completed successfully.") 
        ...
        self._print_log_msg("info","auto_restart -> build command_line class obj")
        command_obj = {
            "auto_restart": True,
            "command": "None",
            "profile": self.thread_profile,  
    ...
    def set_ep(self):  
         # ep: def: edge_point
         self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - setup ep - pulling ep details | profile [{self.node_service.profile}]")
         self.edge_device = self.functions.pull_edge_point(self.node_service.profile)
         self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - setup ep - pulling ep details | remote [{self.edge_device}]")
    ...
    def setup_profile_states(self):
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  setup profiles - initializing known profile sessions and state | all profile [{self.profile_names}]")
        self.profile_states = {}
        ...
        if self.functions.config_obj["global_auto_restart"]["auto_upgrade"]: 
            if not self.functions.config_obj[self.thread_profile]["is_jar_static"]:
                ...
                self._print_log_msg("warning",f"auto_restart - thread [{self.thread_profile}] - update profile states - [auto_upgrade] is [enabled] however, the jar repository is statically defined.  auto_upgrade will not be able to determine if an upgrade is needed, disabling feature [False].")
    ...
    def setup_alerting(self):
        ...
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - alerting module [{self.alerting['enable']}]")
        ...
        if set(necessary_items) != set(found_items):
            self._print_log_msg("error",f"auto_restart - thread [{self.thread_profile}] - invalid alerting configuration found alerting is [disabled]")
            self.alerting = {"enable": False}
            return
        ...
        try:
            ...
            self.alerting = {"enable": False}
            self._print_log_msg("error",f"auto_restart - thread [{self.thread_profile}] - alerting module error in configuration - disabling alerting.")
    ...
    def update_profile_states(self):
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - update profile states | all profile [{self.profile_names}]")
        
        for profile in self.profile_states:
           self.node_service.set_profile(profile)
           self.set_ep()
           self.set_session_and_state()
           self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  update profile states | profile details [{self.profile_states[profile]}]")
                        
           # debugging
           self._print_log_msg("debug","=====================================================================================")
           self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  update profile states | requested by [{self.thread_profile}]")
           self._print_log_msg("debug","=====================================================================================")
           for key,value in self.profile_states[profile].items():
               self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  update profile states | profile [{profile}] {key} [{value}]")
           self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  update profile states | profile [{profile}] cluster version [{self.version_obj[self.environment][self.thread_profile]['cluster_tess_version']}]")
           self._print_log_msg("debug","=====================================================================================")
    ...
    def set_session_and_state(self):
        self._print_log_msg("info",f"auto restart - get session and state - updating profile [{self.node_service.profile}] state and session object with edge device [{self.edge_device}]") 
        ...
        while True: # utilize looper until success
            try:
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] edge [{self.edge_device}]")
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] profile [{self.node_service.profile}]")
                ...
                self._print_log_msg("error",f"auto_restart - set_session_and_state - thread [{self.thread_profile}] error [{e}]")
                attempts = self.attempts_looper(attempts,"session retrieval",20,3,False)
            else:
                break
            
        self._print_log_msg("debug",f"auto_restart - set sessions - profile [{self.thread_profile}] | session_list | {session_list}")
        ...
        if continue_checking:
            if gl0_dependent_link == "external":
                ...
                if ext_state != "Ready":
                    self._print_log_msg("debug",'fauto_restart - set sessions - profile [{self.thread_profile}] | gl0 link not ready | state [{ext_state}] - Detected external link not local to node - setting layer0_wait flag')
                    self.profile_states[self.node_service.profile]["match"] = False
                    self.profile_states[self.node_service.profile]["ep_ready"] = True
                ...
                self._print_log_msg("debug",'fauto_restart - set sessions - profile [{self.thread_profile}] | gl0 link not ready | state [{self.profile_states[gl0_dependent_link]["node_state"]}] - setting layer0_wait flag')
                self.profile_states[self.node_service.profile]["match"] = False
                self.profile_states[self.node_service.profile]["ep_ready"] = True
                self.profile_states[self.node_service.profile]["action"] = "layer0_wait"
                self.profile_states[self.node_service.profile]["node_state"] = session_list["state1"]
            ...
            if self.profile_states[self.node_service.profile]["layer"] > 0:
                if ml0_dependent_link == "external":
                    ...
                    self._print_log_msg("debug",'fauto_restart - set sessions - profile [{self.thread_profile}] | ml0 link not ready | state [{self.profile_states[ml0_dependent_link]["node_state"]}] - setting layer1_wait flag')
                    self.profile_states[self.node_service.profile]["match"] = False
                    self.profile_states[self.node_service.profile]["ep_ready"] = True
                    self.profile_states[self.node_service.profile]["action"] = "layer1_wait"
                    self.profile_states[self.node_service.profile]["node_state"] = session_list["state1"]
        ...
        if continue_checking:                                        
            if session_list["session0"] > session_list["session1"] and session_list['session1'] > 0:
                ...
                if self.stuck_in_state_handler(state):
                    self._print_log_msg("warning",f"auto_restart - thread [{self.thread_profile}] -  set session detected profile [{self.node_service.profile}] - stuck in [{session_list['state1']}] - forcing restart.")
                    self.profile_states[self.node_service.profile]["action"] = "restart_full"  
                    self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  set session detected profile [{self.node_service.profile}] - resetting tolerance counter.")
    ...
    def attempts_looper(self,attempts,action,sec,max_attempts,critical_sleep):
        ...
        if attempts > max_attempts:
            self._print_log_msg("critical",f"auto_restart - thread [{self.thread_profile}] - attempts looper - service has attempted [{max_attempts}] times | profile [{self.node_service.profile}] | action [{action}]")
            self._print_log_msg("warning",f"auto_restart - thread [{self.thread_profile}] -  attempts looper...  profile [{self.node_service.profile}] - LOOPER will wait 10 minutes before continuing.")
            if critical_sleep:
                sleep(self.sleep_on_critical)
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - attempts looper - profile [{self.node_service.profile}] has completed a 10 minute wait and will continue...")
            self.stop_or_start_failed = {
                "failed": True,
                "action": None,
            }
        ...
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  attempts looper - profile [{self.node_service.profile}] - [{action}] attempt [{attempts}] of [{max_attempts}] - pausing {sec}")
        sleep(sec)
        return attempts+1
           
                         
    def wait_for_ep_looper(self,action=None,attempts=1):
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  wait for ep looper - timer invoked thread [{self.node_service.profile}]")
        while True:
            if action == None:
            ...
            if "ep_wait" in action:
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  check and wait for LB - found [{action}]  | error or maintenance [{remote_session}] - entering attempts looper")
                ...
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  check and wait for LB - found [{action}]  | returning from looper")
                return
    ...
    def observing_looper(self):
        ...
        while True:
            ...
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  Observing/WaitingForReady looper - action [{self.node_service.profile}] initiated | current found state [{state}]")
            if state == "Observing" or state == "WaitingForReady":
                ...
                if attempts > max_attempts:
                    self._print_log_msg("error",f"auto_restart - thread [{self.thread_profile}] -  Observing/WaitingForReady looper - action [{self.node_service.profile}] initiated | could not achieve \"Ready\" | current found state [{state}]")
                    return False
            else:
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  Observing/WaitingForReady looper - action [{self.node_service.profile}] exiting | current found state [{state}]")  
                return True
    ...
    def check_service_state(self):
        ...
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  check service state - found service [{self.node_service.profile}] state [{self.node_service.node_service_status[self.node_service.profile]}]")
    ...
    def silent_restart(self,action):
        ...
        if action != "join_only":
            ...
            if self.profile_states[self.node_service.profile]["action"] == "NoActionNeeded":
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  possible cluster restart false positive detected - skipping restart | profile [{self.node_service.profile}]")
                return
            
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  cluster restart false positive detection test cleared - continuing with restart | profile [{self.node_service.profile}]")        
            self._print_log_msg("warning",f"auto_restart - thread [{self.thread_profile}] -  silent restart [stop] initiating | profile [{self.node_service.profile}]")
            self.stop_start_handler("stop")
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  silent restart - updating [seed_list]")
            ...
            # self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  silent restart - sleeping [{self.silent_restart_timer}]")
            # sleep(self.silent_restart_timer)   # not truly necessary but adding more delay
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  silent restart [start] initiating | profile [{self.node_service.profile}]")
            self.stop_start_handler("start")
            
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  silent restart [join] initiating | profile [{self.node_service.profile}]")
        if self.stop_or_start_failed["failed"] == False:
            self.join_handler()
        else:
           self._print_log_msg("debug",f"auto_restart - silent restart - action [{self.stop_or_start_failed['action']}] failed, dropping back to - restart handler")
            
        self._print_log_msg("info",f"auto_restart - silent restart method completed | [{self.node_service.profile}]")
            
                
    # HANDLERS
    def stuck_in_state_handler(self,state):
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - stuck_in_state_handler - testing timing for state [{state}]")
        current_time = time()
        ...
        if not self.stuck_timers[f"{state}_state_enabled"]:
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - stuck_in_state_handler - enabling timer for state [{state}]")
        ...
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - stuck_in_state_handler - timer for state [{state}] reached [{elapsed_seconds}] (will be negative for first run)")
        if elapsed_seconds > self.stuck_timers[f"{state}_tolerance"]:
            return True
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - stuck_in_state_handler - does not meet tolerance level for state [{state}] continuing... tolerance [{elapsed_seconds}] of [{self.stuck_timers[f'{state}_tolerance']}]")
        return False
    ...
    def fork_handler(self,fork_type):
        ...
        if self.fork_check_time[fork_type] > -1: 
            if self.profile_states[self.node_service.profile][fork_type] == "disabled":
                self._print_log_msg("debug",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] fork detection disabled, skipping...")
                return
        
        self._print_log_msg("info",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}]")
        if self.thread_layer > 0:
            self._print_log_msg("debug",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] - layer [{self.thread_layer}] - skipping minority fork detection")
            return
        
        if self.profile_states[self.thread_profile]["node_state"] != "Ready": 
            self._print_log_msg("warning",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] | node state: [{self.profile_states[self.thread_profile]['node_state']}] will not check for fork, skipping.")
            return

        if self.fork_check_time[fork_type] > -1: 
            if self.profile_states[self.node_service.profile][fork_type]:
                if self.fork_check_time[fork_type] >= time() - self.fork_timer:
                    self._print_log_msg("debug",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] - minority check timer not met, skipping.")
                    return False
                else:
            ...
            self._print_log_msg("debug",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] fork detection initializing...")
        ...
        self._print_log_msg("debug",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] checking for {fork_type} fork.")
        ...
        try:
            ...
            if fork_type == "minority_fork":
                self._print_log_msg("error",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] - error while obtaining global ordinals [{e}] skipping check. | nodectl error_code [as-578]")
            elif fork_type == "consensus_fork":    
                self._print_log_msg("error",f"auto_restart - {fork_type}_handler - thread [{self.thread_profile}] - error while checking for consensus fork [{e}] skipping check. | nodectl error_code [as-580]")
            skip = True
        ...
        if fork_type == "minority_fork":
            for key, value in global_ordinals.items():
                if value == None:  
                    skip = True
                    self._print_log_msg("warning",f'auto_restart - minority_fork_handler - thread [{self.thread_profile}] -requesting ordinal resulted in no results - ordinal key/value [{key}/{value}]')
            if skip: return False # skipping restart
            
            self._print_log_msg("debug",'fauto_restart - minority_fork_handler - thread [{self.thread_profile}] - localhost ordinal [{global_ordinals["local"]["ordinal"]}]')
            self._print_log_msg("debug",'fauto_restart - minority_fork_handler - thread [{self.thread_profile}] -   Backend ordinal [{global_ordinals["backend"]["ordinal"]}]')
            self._print_log_msg("debug",'fauto_restart - minority_fork_handler - thread [{self.thread_profile}] - localhost hash [{global_ordinals["local"]["lastSnapshotHash"]}]')
            self._print_log_msg("debug",'fauto_restart - minority_fork_handler - thread [{self.thread_profile}] -   Backend hash [{global_ordinals["backend"]["lastSnapshotHash"]}]')
            
            if global_ordinals["local"]["lastSnapshotHash"] == global_ordinals["backend"]["lastSnapshotHash"]: 
                self._print_log_msg("debug",'fauto_restart - minority_fork_handler - thread [{self.thread_profile}] - fork not detected - valid match found')
                self.profile_states[self.node_service.profile][fork_type] = False
                return False
            ...
            if skip: return False # skipping restart
            if consensus_match:
                self._print_log_msg("debug",'fauto_restart - consensus_fork_handler - thread [{self.thread_profile}] - fork not detected - valid participation detected')
                self.profile_states[self.node_service.profile][fork_type] = False
                return False
        
        # restart needed
        self._print_log_msg("warning",f'auto_restart - {fork_type}_handler - thread [{self.thread_profile}] - fork detected.')
    ...
    def v2v3_migration_handler(self):
        if not self.auto_upgrade: 
            return
        self.version_obj = self.versioning.get_cached_version_obj()
        self._print_log_msg("info",f"auto_restart - thread [{self.thread_profile}] - v2 to v3 migration handler - initialized.")
        for n in range(0,4):
            ...
            if migration_success == "not_needed":
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - v2 to v3 migration handler - not needed, skipping.")
                return                
            if migration_success: 
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - v2 to v3 migration handler - successfully updated.")
                return
            if n > 2:
                self._print_log_msg("critical",f"auto_restart - thread [{self.thread_profile}] - v2 to v3 migration handler - unable to properly migrate this node - skipping.")
                return
            

    def version_check_handler(self,sessionStartedState=False):
        # do not do anything until the versions match
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  version check handler - initiated - profile [{self.node_service.profile}] ")
        attempts = 1
        notice_warning = ""
        warning = False
        ...
        def update_version_cache():
            for n in range(1,5):
                ...
                try:
                    ...
                    self._print_log_msg("critical",f"auto_restart -> version_check_handler -> update_version -> failed with [{e}]")
                    if n > 3:
                        self.attempts_looper(0,"versioning_update",125,1,True)
                    self._print_log_msg("debug",f"attempting to update version object | attempt [{n}] or [3]")
        ...
        while True:
            ...
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  version check handler - version check")
            try:
                if self.version_obj[self.environment][self.thread_profile]["tess_uptodate"] == "true": 
                    self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  version check handler - profile [{self.thread_profile}] - versions matched | Hypergraph/metagraph/cluster [{versions[0]}] node [{versions[1]}]")
                    if not self.auto_upgrade or auto_upgrade_success:
                        return True
                    elif self.auto_upgrade and not auto_upgrade_success:
                        self._print_log_msg("error","auto_restart - auto_upgrade - was unsuccessful in attempt to download new version.")
                    warning = True
            except Exception as e:
                self._print_log_msg("critical",f"auto_restart - thread [{self.thread_profile}] -  version check handler - profile [{self.node_service.profile}] - versions do not match - and we received an error [{e}] - sleeping 10m")
                sleep(self.sleep_on_critical) # ten minutes
            self._print_log_msg("warning",f"auto_restart - thread [{self.thread_profile}] -  version check handler - profile [{self.node_service.profile}] - versions do not match - versions found | Hypergraph/metagraph/cluster [{versions[0]}] node [{versions[1]}] - auto_upgrade setting [{str(self.auto_upgrade)}]")

            if self.auto_upgrade:
                notice_warning = "auto_upgrade to obtain "
                if sessionStartedState:
                    self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  version check handler - version changed while in [SessionStarted] updating to [restart_full]")
                ...
                if auto_upgrade_success: 
                    self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  version check handler - versions upgrade successful")
                    update_version_cache()
                    break
                else: warning = True
            else:
                self._print_log_msg("warning",f"auto_restart - thread [{self.thread_profile}] - auto_upgrade disabled.")
    ...
    def stop_start_handler(self,action):
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start  - profile: [{self.node_service.profile}] api_port [{self.node_service.api_ports['public']}] -- ready to [{action}]")
        if not self.debug:
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start  - profile: [{self.node_service.profile}] [{action}] action commencing...")
        ...
        max_attempts = 10
        while True:
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - profile [{self.node_service.profile}] testing or retesting state")
            self.update_profile_states()
            state = self.profile_states[self.node_service.profile]['node_state'] # readability
            
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - profile [{self.node_service.profile}] found state [{state}]")
            
            if action == "stop":
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - state [{state}] waiting for [stop] to reach [ApiNotReady] or [Offline]")
                if state == "ApiNotReady" or state == "Offline":
                    self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] - stop start - profile [{self.node_service.profile}] stop start exiting [stop] found state [{state}]")
                    break
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - profile [{state}] did not reach desired state [ApiNotReady] or [Offline]")
                
            elif action == "start":
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - waiting for [start] to complete with desired state [ReadyToJoin]")
                ...
                if "Ready" in state or "Observing" in state: # Ready, Observing, WaitingForReady, WaitingForObserving
                    self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - profile [{self.node_service.profile}] stop start exiting [start] found state [{state}]")
                    break
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - profile [{self.node_service.profile}] did not reach desired state [ReadyToJoin]")
            
            attempts = self.attempts_looper(attempts,action,10,max_attempts+1,False)
            if self.stop_or_start_failed["failed"]:
                self._print_log_msg("warning",f"auto_restart - thread [{self.thread_profile}] -  stop start - profile [{self.node_service.profile}] failed [{state}]")
                self.stop_or_start_failed = {
                    "failed": True,
                    "action": action
                }
        ...
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  stop start - action [{action}] [{self.node_service.profile}] api_port [{self.node_service.api_ports['public']}] completed")
    ...
    def join_handler(self):
        ...
        self._print_log_msg("info",f"auto_restart - thread [{self.thread_profile}] -  join handler - [join] action [{self.node_service.profile}] initiated | current found state [{state}]")
        
        # The stop and start procedures would not allow us to get here without being "ReadyToJoin"
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  join handler - starting action [join] | state is [{state}]")
        
        if not self.independent_profile: # we do not need to test other layers before joining
            try:
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  join handler - action [join] | detected layer1 [{self.node_service.profile}] - waiting for layer0 to become ready entering Observing/WaitingForReady state looper")
                observing_to_ready = self.observing_looper()
                if not observing_to_ready:
                    self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  join handler - action [join] | detected layer1 [{self.node_service.profile}] - failed to see L0 set to [Ready] returning to restart handler")
                    return
            except:
        ...
        attempts = 1
        while True:
            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  join handler | profile: [{self.node_service.profile}] [127.0.0.1] port [{self.node_service.api_ports['public']}] - testing or retesting state...")
            self.update_profile_states()
            state = self.profile_states[self.node_service.profile]['node_state'] # readability

            self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  join handler - profile [{self.node_service.profile}] found | state [{state}]")
            
            if "Ready" in state or "Observing" in state: # Ready, Observing, WaitingForReady, WaitingForObserving  
                self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  join handler - success achieved | state [{state}]")
                break
            
            self._print_log_msg("warning",f"auto_restart join handler - profile [{self.node_service.profile}] state in [{state}] entering retry looper")
            ...
            if attempts > 4:
                if "download" in state.lower() or "waiting" in state.lower() or "observing" in state.lower():
                    self._print_log_msg("warning",f"auto_restart join handler - profile [{self.node_service.profile}] not ready to continue state [{state}] dropping back into restart handler.")
                else:
                    self._print_log_msg("warning",f"auto_restart join handler - profile [{self.node_service.profile}] failed, dropping back to restart handler")
                break
        
        
        self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  join handler - completed join attempt | state [{state}] | sleeping [{self.join_pause_timer}] seconds before continuing")
        sleep(self.join_pause_timer)
        self.update_profile_states()
        state = self.profile_states[self.node_service.profile]['node_state'] # readability
        self._print_log_msg("info",f"auto_restart - thread [{self.thread_profile}] -  join handler - check for ready state | final state: [{state}] [127.0.0.1] port [{self.node_service.api_ports['public']}] - exiting join_handler")  
    

    def alert_handler(self):
        if not self.alerting["enable"]: 
            self._print_log_msg("debug",f"auto_restart -> thread [{self.thread_profile}] --> alerting is disabled")
            return
        ...
        if self.alerting["begin_alert_utc"] == self.alerting["end_alert_utc"]:
            self._print_log_msg("debug",f"auto_restart -> thread [{self.thread_profile}] -> alerting set to always on.")
            ...
            if not (self.alerting["begin_alert_utc"] <= report_hour < self.alerting["end_alert_utc"]):
                self._print_log_msg("debug",l_msg)
                return  
        ...
        if self.profile_states[self.thread_profile]["action"] == "NoActionNeeded":
            try:
                ...
                self._print_log_msg("error",f"auto_restart - thread [{self.thread_profile}] - alert handler - unable to send report - issue with configuration settings")
            return
    ...
    def restart_handler(self):
        ...
        self._print_log_msg("info",f"auto_restart - thread [{self.thread_profile}] -  restart handler - invoked | profile [{self.thread_profile}]")

        while True:
            if self.passphrase_warning:
                self._print_log_msg("error",f"auto_restart - restart handler -  thread [{self.thread_profile}] found possible manual passphrase requirement - auto_restart will not be able to proper authenticate on restart requirement, sleeping [20 minutes].")
                sleep(1200)
                self._print_log_msg("info",f"auto_restart - restart handler -  thread [{self.thread_profile}] re-checking if configuration has been updated...")
                self.setup_profile_states()
            else:
                self._print_log_msg("info",f"auto_restart - thread [{self.thread_profile}] -  restart handler - timer invoked thread [{self.thread_profile}] - sleeping [{self.timer}] seconds")
                sleep(self.timer)
                self._print_log_msg("info",f">>>> auto_restart - thread [{self.thread_profile}] -  restart handler - thread [{self.thread_profile}] timer expired [{self.timer}] seconds elapsed, checking sessions.") 
                ...
                if action == "ep_wait":
                    ...
                    warn_msg += "=========================================================================="
                    self._print_log_msg("warning",warn_msg)
                    ...
                    warn_msg += "=========================================================================="
                    self._print_log_msg("info",warn_msg)
                    ...
                    # self._print_log_msg("debug",f"auto_restart - thread [{self.thread_profile}] -  restart handler - random sleep before executing restart request | sleep [{extra_wait_time}s]")
