...
def enable_disable_ipv6(action, log, functions, non_interactive):
    log.logger.info(f"enable_disable_ipv6 -> modify [/etc/sysctl.conf] to disable IPv6.")
...
'''
    ...
    log.logger.info("enable_disable_ipv6 -> checking if IPv6 settings are already present in sysctl.conf")
    with open(sysctl_conf, 'r') as f:
        content = f.read()

    if action == "disable" and 'disable_ipv6' not in content:
        need_reboot = True
        log.logger.info("enable_disable_ipv6 -> Appending IPv6 disable settings to [sysctl.conf]")
        ...
        log.logger.warning("enable_disable_ipv6 -> Not updating IPv6 disable settings to sysctl.conf because the configuration is already in place.")
    ...
    if apply:
        need_reboot = True
        log.logger.info("enable_disable_ipv6 -> Apply IPv6 in sysctl changes.") 
    ...
    functions.print_cmd_status(ipv6_obj)
    log.logger.info("enable_disable_ipv6 -> Update GRUB to disable IPv6 at the kernel level.")
    ipv6_obj["newline"] = True
    ...
    if action == "disable" and 'ipv6.disable=1' not in grub_content:
        log.logger.info("enable_disable_ipv6 -> Modify GRUB to disable IPv6.")
        new_grub_content = grub_content.replace(
            'GRUB_CMDLINE_LINUX_DEFAULT="',
            'GRUB_CMDLINE_LINUX_DEFAULT="ipv6.disable=1 '
        )

        log.logger.info("enable_disable_ipv6 -> Applying GRUB config.")
        ...
        log.logger.warning("enable_disable_ipv6 -> Not necessary to modify GRUB; IPv6 already disabled.")
        ipv6_obj["status"] = "NotNeeded"
        ipv6_obj["status_color"] = "green"
        apply = False
    ...
    if apply:
        ...
        log.logger.info("enable_disable_ipv6 -> GRUB updated to disable IPv6 at boot level.")
        ipv6_obj["status"] = "Completed"
        ipv6_obj["status_color"] = "green"
