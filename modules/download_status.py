...
class DownloadStatus():
    ...
    def pull_local_dip_details(self):
        ...
        try:
            with ThreadPoolExecutor() as executor:
                if self.initialize:
                    ...
                    _ = executor.submit(self.functions.print_spinner,{
                        "msg": f"Loading DownloadingProgress details",
                        "color": "cyan",
                    })     
            ...
            self.log.logger.warn(f"download_status -> Unable to open JSON from log -> decoding error: [{e}]") 
    ...
    def build_percent_hashes(self):
        ...
        try:
            ...
            # length = len(self.dip_vals.hash_marks)
            # self.dip_vals.hash_marks = f"{self.dip_vals.hash_marks:>{length}}"
